...
ggplot(combined.df, aes(x = date_time, y = temp_top, colour = tank)) +
  ...
  scale_x_datetime(date_breaks = "4 days", date_labels = "%d %b",
                   limits = c(as_datetime(a_month_ago), NA)) +
  ylim(-180, -160) +
  facet_grid(rows = vars(tank))
...
The level of liquid nitrogen in the tanks changes as it evaporates away and is refilled. The minimum level triggering a fill is 205 mm, tanks get filled to 250 mm. On 20th September 2022 (red dotted line) the manifold was commissioned allowing "one-fill all-fill" (OFAF) to work. OFAF fills all tanks sequentially once any one triggers the process.
...
ggplot(combined.df, aes(x = date_time, y = level, colour = tank)) +
  ...
  scale_x_datetime(limits = c(as_datetime(c(a_month_ago, NA))),
    date_breaks = "4 days",
    date_labels = "%d %b") +
  facet_grid(rows = vars(tank))
...
```{r nitrogen usage boxplot}
#| echo: false
#| message: false
#| warning: false

ggplot(combined.df, aes(x = tank, y = usage, colour = tank)) +
  theme_bw() +
  labs(title = "Nitrogen usage since install") +
...
The tables below list the last 10 status messages for each tank, removing the very common filling, hot gas bypass, and level zeroing codes. A list of the meaning of all event log codes is at the bottom of the page. The most recent event is first.
